  ...
  switch (ctx.players.length) {
    ...
    case 2: 
      return (
        <div className="game-table">
          <div className="row">
            <div className={"col-2" + (ctx.turnIndex === 1 ? ' active' : '')}>
              <Seat key="1"
                    position="left" 
          ...
          <div className={"row" + (ctx.turnIndex === 0 ? ' active' : '')}>
            <Seat key="0"
                    position="bottom" 
                    player={ctx.players[0]} 
                    selectedCards={ctx.selectedCards} 
      ...
      return (
        <div className="game-table">
            ...
            <div className="row center-row justify-content-center">
                ...
                <div className="col-8 justify-content-center reveal-cards">
                    <RevealCards cards={ctx.revealCards} 
    ...
    case 5:
     return (
        <div className="game-table">
          <div className="row">
            <Seat key="1" position="top-1" player={ctx.players[2]}    
                      selectedCards={ctx.selectedCards} 
                      turnIndex={ctx.turnIndex} />
            <Seat position="top-2" player={ctx.players[3]}    
                      selectedCards={ctx.selectedCards} 
                      turnIndex={ctx.turnIndex} />
          </div>
          <div className="row">
            <div className="col-2">
                <Seat key="2" position="left" player={ctx.players[1]}    
                      selectedCards={ctx.selectedCards} 
                      turnIndex={ctx.turnIndex} />
            </div>
            <div className="col-8">
                <RevealCards cards={ctx.revealCards}     
                      selectedCards={ctx.selectedCards} turnIndex={ctx.turnIndex} />
            </div>
            <div className="col-2">
                <Seat key="3" position="right" player={ctx.players[3]}     
                      selectedCards={ctx.selectedCards} 
                      turnIndex={ctx.turnIndex} />
            </div>  
          </div>
          <div>
            <Seat key="0" position="bottom" player={ctx.players[0]}     
                      selectedCards={ctx.selectedCards} 
                      turnIndex={ctx.turnIndex} />
          </div>
        </div>
    ...
    case 6:
      return (
        <div className="game-table">
          <div className="row">
            <Seat key="3" position="top" player={ctx.players[3]}    
                      selectedCards={ctx.selectedCards} 
                      turnIndex={ctx.turnIndex} />
          </div>
          <div className="row">
            <div className="col-2">
                <div className="row">
                    <Seat key= "1" position="left-1" player={ctx.players[1]}    
                      selectedCards={ctx.selectedCards} 
                      turnIndex={ctx.turnIndex} />
                </div>
                <div className="row">
                    <Seat key="2" position="left-2" player={ctx.players[2]}    
                      selectedCards={ctx.selectedCards} 
                      turnIndex={ctx.turnIndex} />
                </div>
            </div>
            ...
            <div className="col-2">
                <div className="row">
                    <Seat key="4" position="right-1" player={ctx.players[4]}    
                      selectedCards={ctx.selectedCards} 
                      turnIndex={ctx.turnIndex} />
                </div>
                <div className="row">
                    <Seat key="5" position="right-2" player={ctx.players[5]}    
                      selectedCards={ctx.selectedCards} 
                      turnIndex={ctx.turnIndex} />
                </div>
            </div>
...
function Seat({ position, player }) {
  ...
  const ctx = useContext(UserContext);
    ...
    switch (position) { 
        ...
        case 'left-2':
            return (
                 <div className="row seat left align-items-center">
                    <div className="card-container vertical col-6 seat left-side" >
                        {player.hand.map((card) => (
                            <Card key={card.id} 
                                  position={position} 
                                  card={card}
                   ...
                   <div className="card-container vertical col-6 trios left" >
                     {player.trios.map((triosSet, indx) => (
                      <div key={position + "-" + indx.toString()} className="trioSet" >
                        {triosSet.map((card, indx) => (
                            <Card key={card.id} 
        ...
        case 'right-2':
            return (
                <div className="row seat right align-items-center">
                   <div className="card-container vertical col-6 trios right" >
                     {player.trios.map((triosSet, indx) => (
                      <div key={position + "-" + indx.toString()} className="trioSet" >
                        {triosSet.map((card, indx) => (
                            <Card key={card.id} 
                ...
                <div className="card-container vertical col-6 seat right-side" >
                    {player.hand.map((card) => (
                            <Card key={card.id} 
                                  position={position} 
                                  card={card}
...
function Card({ card, position,isSelectable, isSelected, isHuman, overlap, isTrio}) {
  ...
  const ctx = useContext(UserContext);
    ...
    if (isSelected) {
        return (
          <div className={'game-card selected' + (isHuman ? ' human' : ' bot') + (card.id > 35 ? ' hidden' : '')} id={'card-' + card.id} onClick={() => ctx.handleCardClick(card, isSelected)}>
        ...
        if (isSelectable) {
            return (
              <div className={'game-card selectable' + (isHuman ? ' human' : ' bot') + (card.id > 35 ? ' hidden' : '')} id={'card-' + card.id} onClick={() => ctx.handleCardClick(card, isSelected)}>
                <span>{card.rank}</span>
            ...
            return (
              <div className={'game-card' + (isHuman ? ' human' : ' bot') + (overlap ? ' overlap' : '') + (card.id > 35 ? ' hidden' : '')} 
              id={'card-' + card.id} onClick={() => ctx.handleCardClick(card, isSelected)}>
                <span className='tmp'>{card.rank}</span>
                <span className="rank">{isHuman || isTrio ? card.rank.toString() : ''}</span>
              </div>
            );   
        }
    }
